...
(mfn/deftest with-mainloop-test
  (mfn/testing "on initialization and termination"
    (mfn/testing "produce events to handler function"
      ...
      (mfn/verifying
        (handle-fn {:type :init :value true}) nil (mfn.m/exactly 1)
        (handle-fn {:type :halt :value true}) nil (mfn.m/exactly 1)))

    (mfn/testing "produce events to render function"
      (let [state (atom {})]
        (mainloop/with-mainloop handle-fn render-fn state [] output!))
      (mfn/verifying
        (render-fn output-buffer {} {:init true}) nil (mfn.m/exactly 1)
        (render-fn output-buffer (mfn.m/any) {:init true :halt true}) nil (mfn.m/exactly 1))))
  ...
  (mfn/testing "on changes to the state atom"
    (mfn/testing "invokes the render function on each change"
      (let [state (atom {})
        ...
        (mainloop/with-mainloop handle-fn render-fn state ['ev1 'ev2] output!))
      (mfn/verifying
        (render-fn output-buffer (mfn.m/any) {:init true :last-event 'ev1}) nil (mfn.m/exactly 1)
        (render-fn output-buffer (mfn.m/any) {:init true :last-event 'ev2}) nil (mfn.m/exactly 1)
    ...
    (mfn/testing "no longer invokes render function when mainloop is over"
      ...
      (mfn/verifying
        (render-fn output-buffer (mfn.m/any) {:init true}) nil (mfn.m/any)
        (render-fn output-buffer (mfn.m/any) {:init true :halt true}) nil (mfn.m/any))))
  ...
  (mfn/testing "on terminal dimensions"
    ...
    (mfn/testing "notifies the handler function on init/resize"
      (let [state (atom {})]
        (mainloop/with-mainloop handle-fn render-fn state [{:type :cursor-position :value [10 20]}] output!))
      (mfn/verifying
        (handle-fn {:type :size :value [10 20]}) nil (mfn.m/exactly 1)
        (handle-fn mfn.m/any-args?) nil (mfn.m/any)))
